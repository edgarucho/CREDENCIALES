...
(function (global) {
  ...
  function cargarDatos() {
    var guardado = leerLocal();
    if (guardado && guardado.escuelas && guardado.escuelas.length) return guardado;
    var base = global.PLANTILLAS_BASE;
    return base ? clonar(base) : { version: 2, servidor: '', escuelas: [] };
  }

  function guardarDatos(d) { return escribirLocal(d); }
  ...
  function envolver(ctx, texto, ancho) {
    ...
    for (var i = 0; i < palabras.length; i++) {
      ...
      if (ctx.measureText(prueba).width > ancho && actual) { lineas.push(actual); actual = palabras[i]; }
      else actual = prueba;
    }
  ...
  function dibujarCampo(ctx, campo, valor, W, H) {
    ...
    if (campo.tipo === 'parrafo') {
      ...
      for (var i = 0; i < 40; i++) {
        ...
        if (lineas.length * t * 1.18 <= h || t <= px * 0.45) break;
        t = t * 0.94;
      }
      ...
      var tf = encogerLinea(ctx, texto, w, px, fuente, peso);
      ctx.font = peso + ' ' + tf + 'px ' + fuente;
      ctx.fillText(texto, ax, y + h / 2);
    }
    ctx.restore();
  }
