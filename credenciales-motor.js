...
(function (global) {
  ...
  function dibujarCampo(ctx, campo, valor, W, H) {
    ...
    if (campo.tipo === 'foto' || campo.tipo === 'firma') {
      ...
      if (campo.tipo === 'firma') {
        // Brillo y contraste: sirven para limpiar una firma fotografiada
        // en papel (subir contraste borra el gris del fondo).
        var brf = (valor.brillo == null ? 100 : valor.brillo);
        var cof = (valor.contraste == null ? 100 : valor.contraste);
        if (brf !== 100 || cof !== 100) {
          try { ctx.filter = 'brightness(' + (brf/100) + ') contrast(' + (cof/100) + ')'; } catch(e){}
        }
        ...
        // Brillo y contraste (100 = normal). Se aplican como filtro profesional.
        var br = (valor.brillo == null ? 100 : valor.brillo);
        var co = (valor.contraste == null ? 100 : valor.contraste);
        if (br !== 100 || co !== 100) {
          try { ctx.filter = 'brightness(' + (br/100) + ') contrast(' + (co/100) + ')'; } catch(e){}
        }
  ...
  function dibujarFotoPreview(canvas, campo, valor, opciones) {
    ...
    var im = cache[valor.src];
    if (!im) { return; }
    var x = 0, y = 0, w = ancho, h = alto;
    ctx.save();
    ctx.beginPath(); ctx.rect(x, y, w, h); ctx.clip();

    // La firma se dibuja como en la credencial: completa, sin recortar
    if (campo.tipo === 'firma') {
      var brf = (valor.brillo == null ? 100 : valor.brillo);
      var cof = (valor.contraste == null ? 100 : valor.contraste);
      if (brf !== 100 || cof !== 100) {
      ...
      var ef = Math.min(w / im.width, h / im.height);
      var dwf = im.width * ef, dhf = im.height * ef;
      ctx.drawImage(im, x + (w - dwf) / 2, y + (h - dhf) / 2, dwf, dhf);
      ctx.restore();
      return;
    }
    ...
    var br = (valor.brillo == null ? 100 : valor.brillo);
    var co = (valor.contraste == null ? 100 : valor.contraste);
    if (br !== 100 || co !== 100) { try { ctx.filter = 'brightness(' + (br/100) + ') contrast(' + (co/100) + ')'; } catch(e){} }
    var ca = Math.abs(Math.cos(ang)), sa = Math.abs(Math.sin(ang));
    var Wp = w * ca + h * sa, Hp = w * sa + h * ca;
    var rc = Math.max(Wp / w, Hp / h);
    var ec = Math.max(w / im.width, h / im.height) * z * rc;
  ...
  global.CEDULA = {
    PPP: PPP, holguraFoto: holguraFoto,
