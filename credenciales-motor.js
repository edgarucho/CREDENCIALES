...
(function (global) {
  ...
  function dibujarCampo(ctx, campo, valor, W, H) {
    ...
    /* VALOR FIJO: los que se marcan como "lo lleno yo" salen SIEMPRE,
       venga o no venga el dato guardado.

       Antes solo se ponía al armar la página del cliente. Si el campo se
       agregaba después de creada la credencial, o si se hacía desde el
       estudio, el valor no llegaba y el renglón salía en blanco.
       Aquí se resuelve para todos los caminos de una vez.              */
    if (campo.fijo) {
      var vf = (campo.valorFijo == null ? '' : String(campo.valorFijo));
      if (vf !== '') valor = vf;
    }
